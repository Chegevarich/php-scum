<?php 

$a = [1,1,1,1,1,1,1,100];

function findIt($array, $sumNeeded) {
	
	if (array_search($sumNeeded, $array) !== false) {
		return true;
	}
	
	$sum = $array[0];
	
	for ($i=0; $i<=(count($array)); $i++) {
		
		if ($sum === $sumNeeded) {
			return true;
		}
		
		//если следующего элемента не существует
		if (empty($array[$i+1])) {
			//если массив долистан до конца
			if (count($array) == 1) {
				return false;
			} else {
				readAgain($i,$array,$sum);
			}
		} else {
		
			if ($sum < $sumNeeded) {
				
				$sum += $array[$i+1];
				
			} else {
				readAgain($i,$array,$sum);
			}	
		}
		
	}
	
	return false;
}

function readAgain(&$i, &$array, &$sum) {
	$i = 0;
	$sum = array_shift($array);
}

print_r(['possible', findIt($a, 3) === true ? 'true' : 'false']);

print_r(['imposible', findIt($a, 999) === true ? 'true' : 'false']);

print_r(['0 to find (impossible)', findIt($a, 0) === true ? 'true' : 'false']);

print_r(['first is the answer', findIt($a, 1) === true ? 'true' : 'false']);

print_r(['last is the answer', findIt($a, 6) === true ? 'true' : 'false']);

print_r(['middle is the answer', findIt($a, 6) === true ? 'true' : 'false']);

print_r(['N < 0 searched', findIt($a, -1) === true ? 'true' : 'false']);
